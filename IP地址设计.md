...
A类地址范围：1 ~ 127；1.0.0.1-127.255.255.255,任何一个0到127间的网络地址均是一个A类地址；***从1算到127（包括1），总数的确是128***，IP地址最后结尾的部分的x.x.x.0即为网络地址，最后结尾的部分以x.x.x.255即为广播地址，二者并不能作为ip使用，因此需要减去2，***即128-2=126***，因此可定义126（127-1）个A类网络，每个网络可以拥有的主机数为2<sup>h</sup>-2=16777214
